...
def rawg_extract(rawg_game: dict, platform_id: int) -> dict:
    ...
    # RAWG metacritic_platforms items shape varies — defensive extraction
    for plat in rawg_game.get("metacritic_platforms", []):
        try:
            plat_field = plat.get("platform")
            if isinstance(plat_field, int):
                plat_id = plat_field
            elif isinstance(plat_field, dict):
                plat_id = plat_field.get("id")
            else:
                continue
